// Debug CLS with network throttling similar to Lighthouse mobile
...
(async () => {
  ...
  for (const [i, e] of result.entries.entries()) {
    ...
    for (const s of e.sources) {
      console.log(
        `   -> ${s.selector.slice(0, 120)}`
      );
      console.log(
        `      prev: x=${Math.round(s.prev?.x || 0)} y=${Math.round(s.prev?.y || 0)} w=${Math.round(s.prev?.width || 0)} h=${Math.round(s.prev?.height || 0)}`
      );
      console.log(
        `      curr: x=${Math.round(s.curr?.x || 0)} y=${Math.round(s.curr?.y || 0)} w=${Math.round(s.curr?.width || 0)} h=${Math.round(s.curr?.height || 0)}`
      );
